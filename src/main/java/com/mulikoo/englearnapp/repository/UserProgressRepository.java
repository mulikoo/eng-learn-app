...
public interface UserProgressRepository extends JpaRepository<UserProgress, Long> {
    ...
    @Query("SELECT up.word.id as wordId, up.status as status FROM UserProgress up WHERE up.user = :user")
    List<UserProgressView> findWordIdsByUser(@Param("user") User user);
